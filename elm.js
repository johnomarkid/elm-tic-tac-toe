...
Elm.Model.make = function (_elm) {
   ...
   var initialBoard = function (numRows) {
      var returnNum = function (_p0) {    return NA;};
      return A2($List.map,function (i) {    return A2($List.map,returnNum,_U.range(0,numRows));},_U.range(0,numRows));
   };
   var initialModel = F2(function (bs,rs) {    return {board: initialBoard(bs),turn: X,status: Ongoing,boardSize: bs,randSeed: rs};});
   var Model = F5(function (a,b,c,d,e) {    return {board: a,turn: b,status: c,boardSize: d,randSeed: e};});
   return _elm.Model.values = {_op: _op,Model: Model,initialModel: initialModel,initialBoard: initialBoard,Ongoing: Ongoing,Tie: Tie,Win: Win,X: X,O: O,NA: NA};
};
...
Elm.GameLogic.make = function (_elm) {
   ...
   var changeTurn = function (model) {
      var newTurn = function () {    var _p0 = model.turn;switch (_p0.ctor) {case "X": return $Model.O;case "O": return $Model.X;default: return $Model.NA;}}();
      return _U.update(model,{turn: newTurn});
   };
   var updateSquare = F2(function (location,model) {
      var newBoard = A3($List.map2,
      F2(function (x,row) {
         return A3($List.map2,
         F2(function (y,item) {    return _U.eq({ctor: "_Tuple2",_0: x,_1: y},location) ? model.turn : item;}),
         _U.range(0,model.boardSize),
         row);
      }),
      _U.range(0,model.boardSize),
      model.board);
      return _U.update(model,{board: newBoard});
   });
   var fromJust = function (x) {
      ...
      if (_p1.ctor === "Just") {
            ...
            return _U.crashCase("GameLogic",{start: {line: 45,column: 3},end: {line: 50,column: 44}},_p1)("error: fromJust Nothing");
         }
   };
   var get = F2(function (n,xs) {    return fromJust($List.head(A2($List.drop,n,xs)));});
   var invertScoreboard = function (board) {
      return A2($List.map,function (i) {    return A2($List.map,function (row) {    return A2(get,i,row);},board);},_U.range(0,$List.length(board) - 1));
   };
   var invertUpVertical = function (board) {
      var boardLength = $List.length(board) - 1;
      return A3($List.map2,F2(function (row,index) {    return _U.eq(A2(get,boardLength - index,row),1) ? 1 : 0;}),board,_U.range(0,boardLength));
   };
   var invertDownVertical = function (board) {
      return A3($List.map2,F2(function (row,index) {    return _U.eq(A2(get,0 + index,row),1) ? 1 : 0;}),board,_U.range(0,$List.length(board) - 1));
   };
   var randLocation = function (model) {
      randLocation: while (true) {
         var randRange = A2($Random.$int,0,$List.length(model.board) - 1);
         var rowResult = A2($Random.generate,randRange,model.randSeed);
         var colResult = A2($Random.generate,randRange,$Basics.snd(rowResult));
         var location = {ctor: "_Tuple2",_0: $Basics.fst(rowResult),_1: $Basics.fst(colResult)};
         var markAtLocation = A2(get,$Basics.snd(location),A2(get,$Basics.fst(location),model.board));
         var newSeed = $Basics.snd(colResult);
         var newModel = _U.update(model,{randSeed: newSeed});
         if (_U.eq(markAtLocation,$Model.NA)) return {ctor: "_Tuple2",_0: location,_1: newModel}; else {
               var _v2 = newModel;
               model = _v2;
               continue randLocation;
            }
      }
   };
   var listToStatus = function (items) {    return _U.eq($List.sum(items),$List.length(items)) ? $Model.Win : $Model.Ongoing;};
   var createScoreboard = F2(function (mark,board) {
      return A3($List.map2,
      F2(function (row,items) {
         return A3($List.map2,F2(function (col,item) {    return _U.eq(item,mark) ? 1 : 0;}),_U.range(0,$List.length(board) - 1),items);
      }),
      _U.range(0,$List.length(board) - 1),
      board);
   });
   var checkWin = function (model) {
      var remainingMoves = $List.sum(A2($List.map,$List.sum,A2(createScoreboard,$Model.NA,model.board)));
      var scoreBoard = A2(createScoreboard,model.turn,model.board);
      var checkRow = A2($List.map,listToStatus,scoreBoard);
      var checkCol = A2($List.map,listToStatus,invertScoreboard(scoreBoard));
      var checkUpVertical = listToStatus(invertUpVertical(scoreBoard));
      var checkDownVertical = listToStatus(invertDownVertical(scoreBoard));
      var status = function (v) {
         return _U.eq(v,true) ? $Model.Win : $Model.Ongoing;
      }(A2($List.member,
      $Model.Win,
      function (v) {
         return A2($List._op["::"],checkDownVertical,v);
      }(function (v) {    return A2($List._op["::"],checkUpVertical,v);}(A2($List.append,checkRow,checkCol)))));
      var newStatus = function () {
         var _p3 = _U.eq(remainingMoves,0);
         if (_p3 === true) {
               return _U.eq(status,$Model.Ongoing) ? $Model.Tie : status;
            } else {
               return status;
            }
      }();
   ...
   return _elm.GameLogic.values = {_op: _op
                                  ...
                                  ,listToStatus: listToStatus
                                  ,get: get
                                  ,fromJust: fromJust
                                  ,invertScoreboard: invertScoreboard
                                  ,invertUpVertical: invertUpVertical
                                  ,invertDownVertical: invertDownVertical
...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Reset": return A2($Model.initialModel,2,model.randSeed);
         case "IncreaseBoard": return A2($Model.initialModel,model.boardSize + 1,model.randSeed);
         default: var newModel = function (bModel) {
              if (!_U.eq(bModel.status,$Model.Ongoing)) return $GameLogic.changeTurn(bModel); else if (_U.eq(bModel.turn,$Model.O)) {
                       var _p1 = $GameLogic.randLocation(bModel);
                       var loc = _p1._0;
                       var newModel = _p1._1;
                       return A2(update,Turn(loc),newModel);
                    } else return bModel;
           }(function (aModel) {    return $GameLogic.changeTurn(aModel);}($GameLogic.checkWin(A2($GameLogic.updateSquare,_p0._0,model))));
           return newModel;}
   });
   var IncreaseBoard = {ctor: "IncreaseBoard"};
   var Reset = {ctor: "Reset"};
   var NoOp = {ctor: "NoOp"};
   var actions = $Signal.mailbox(NoOp);
   var model = A3($Signal.foldp,update,A2($Model.initialModel,2,$Random.initialSeed(6773)),actions.signal);
   ...
   var squareView = F3(function (rowNum,colNum,mark) {
      ...
      var action = _U.eq(mark,$Model.NA) ? Turn(location) : NoOp;
   ...
   var view = F2(function (address,model) {
      ...
      var board = _U.eq(model.status,$Model.Ongoing) ? A2($Graphics$Element.flow,
      $Graphics$Element.down,
      A3($List.map2,rowView(model.boardSize),_U.range(0,model.boardSize),model.board)) : $Graphics$Element.show("Play again!");
      return A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _U.list([board,A2($Graphics$Element.flow,$Graphics$Element.right,buttons),$Graphics$Element.show(statusMessage)]));
   });
