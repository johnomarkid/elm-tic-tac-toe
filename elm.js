...
Elm.Main.make = function (_elm) {
   ...
   var fromJust = function (x) {
      ...
      if (_p0.ctor === "Just") {
            ...
            return _U.crashCase("Main",{start: {line: 126,column: 3},end: {line: 131,column: 44}},_p0)("error: fromJust Nothing");
         }
   };
   ...
   var randLocation = function (model) {
      randLocation: while (true) {
         ...
         if (_U.eq(markAtLocation,NA)) return {ctor: "_Tuple2",_0: location,_1: newModel}; else {
               var _v2 = newModel;
               model = _v2;
   ...
   var changeTurn = function (model) {
      var newTurn = function () {    var _p3 = model.turn;switch (_p3.ctor) {case "X": return O;case "O": return X;default: return NA;}}();
      return _U.update(model,{turn: newTurn});
   };
   var Win = {ctor: "Win"};
   var Tie = {ctor: "Tie"};
   var Ongoing = {ctor: "Ongoing"};
   var listToStatus = function (items) {    return _U.eq($List.sum(items),$List.length(items)) ? Win : Ongoing;};
   var checkWin = function (model) {
      var remainingMoves = $List.sum(A2($List.map,$List.sum,A2(createScoreboard,NA,model.board)));
      ...
      var status = function (v) {
         return _U.eq(v,true) ? Win : Ongoing;
      }(A2($List.member,
      Win,
      function (v) {
      ...
      var newStatus = function () {
         var _p4 = _U.eq(remainingMoves,0);
         if (_p4 === true) {
               return _U.eq(status,Ongoing) ? Tie : status;
            } else {
               return status;
            }
      }();
   ...
   var initialBoard = function (numRows) {
      var returnNum = function (_p6) {    return NA;};
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "Reset": return A2(initialModel,2,model.randSeed);
         case "IncreaseBoard": return A2(initialModel,model.boardSize + 1,model.randSeed);
         default: var newModel = function (bModel) {
              if (!_U.eq(bModel.status,Ongoing)) return changeTurn(bModel); else if (_U.eq(bModel.turn,O)) {
                       var _p8 = randLocation(bModel);
                       var loc = _p8._0;
                       var newModel = _p8._1;
                       return A2(update,Turn(loc),newModel);
                    } else return bModel;
           }(function (aModel) {    return changeTurn(aModel);}(checkWin(A2(updateSquare,_p7._0,model))));
           return newModel;}
   });
   var model = A3($Signal.foldp,update,A2(initialModel,2,$Random.initialSeed(6773)),actions.signal);
